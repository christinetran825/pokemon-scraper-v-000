class Pokemon
  ...
  def initialize(id, name, type, db)  #(id:, name:, type:, db:)
    @id = id
    @name = name
    @type = type
    @db = db
  ...
  #
  # def self.find(id, db)
  #   the_pokemon = @db.execute("SELECT * FROM pokemon WHERE id = ?", id)
  #   Pokemon.new(id: the_pokemon[0], name: the_pokemon[1], type: the_pokemon[2])
  # end
  #
  # #
  # # describe ".find" do
